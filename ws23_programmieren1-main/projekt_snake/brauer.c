...
int slen = START_SCHLANGE_LAENGE;
...
{
    int S = START_SCHLANGE_LAENGE;
    for (int oy=-S/2; oy<=S/2; oy++)
        for (int ox=-S/2; ox<=S/2; ox++)
            spielfeld[sy+oy][sx+ox] = LEER;
}
...
void spielfeld_initialisieren() {
    ...
    spielfeld_hindernisswaende_einfuegen(ANZAHL_VERTIKALE_WAENDE,   0,1);
    spielfeld_hindernisswaende_einfuegen(ANZAHL_HORIZONTALE_WAENDE, 1,0);
    spielfeld_hindernisswaende_einfuegen(ANZAHL_DIAGONALE_WAENDE, -1,1);
...
void setze_schlange_ins_spielfeld()
{
    ...
    spielfeld[sy][sx] = SCHLANGEN_KOPF;
